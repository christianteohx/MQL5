...
input double min_risk_pct_of_balance = 5.0; // Minimum risk as % of balance (5.0 = 5%)
...
double buy_confidence = 0.0;  // Confidence for buy signal
double sell_confidence = 0.0; // Confidence for sell signal
...
void OnTick() {
    ...
    bool buy_price_cross_single = candle[1].open <= first_ema_buffer[1] && candle[1].close > first_ema_buffer[1];

    bool sell_price_cross_single = candle[1].open >= first_ema_buffer[1] && candle[1].close < first_ema_buffer[1];
    ...
    if (ma_strategy == SINGLE_MA) {
        ...
        if (buy_price_cross_double) {
            ...
            if (buy_double_ma) {
                reasoning += " and price crossed above both MAs";
            } else {
                reasoning += "MA: Price crossed above slow MA";
            }
            ...
            if (sell_double_ma) {
                ...
                reasoning += "MA: Price crossed below slow MA";
            }
        }
    ...
    if (rsi_strategy == COMPARISON) {
        ...
        if (currentRSI < rsi_oversold) {
            double range = rsi_oversold - 0.0; // Range from 0 to oversold level

            confidenceRSI = ((rsi_oversold - currentRSI) / range) * 0.7 + 0.3; // Scale confidence from 0 to 0.7
            buy_rsi = true;
            sell_rsi = false;
            ...
            confidenceRSI = -((currentRSI - rsi_overbought) / range) * 0.7 - 0.3; // Scale confidence from -0.7 to -1.0
            buy_rsi = false;
            sell_rsi = true;

            reasoning += StringFormat("RSI: Extremely overbought (%.2f >= %.2f, Confidence: %.2f)\n", currentRSI, rsi_overbought, confidenceRSI);
        } else { // Intermediate values: interpolate using 50 as pivot
            ...
            if (currentRSI < 50.0) {
                ...
                confidenceRSI = ((50.0 - currentRSI) / range) * 0.3;

                // Clip in case of rounding issues
                if (confidenceRSI > 1.0)
                    confidenceRSI = 1.0;
                ...
                confidenceRSI = -((currentRSI - 50.0) / range) * 0.3;
...
void BuyAtMarket(string comments = "") {
    ...
    if (atr_strategy == USE_ATR) {
        if (atr_sl_multiplier > 0)
            sl = NormalizeDouble(RoundToTick(tick.bid - (current_atr * atr_sl_multiplier)), decimal); // Adjust for bid at exit
        else
...
void SellAtMarket(string comments = "") {
    ...
    if (atr_strategy == USE_ATR) {
        if (atr_sl_multiplier > 0)
            sl = NormalizeDouble(RoundToTick(tick.ask + (current_atr * atr_sl_multiplier)), decimal); // Adjust for ask at exit
        else
    ...
    if (MathAbs(change) > percent_change) {
        ...
        if (last_close_position.buySell == NULL) {
            if (change > 0) {
                ...
                BuyAtMarket("Continue buy"); // Pass 0.0 as ATR if not used
            } else {
                // printf("Selling after %.2f%% change", change);
                SellAtMarket("Continue sell"); // Pass 0.0 as ATR if not used
            }
...
void updateSLTP(double current_atr) {
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (!PositionSelectByTicket(position_ticket))
            continue; // Ensure position is selected
        ...
        if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
            ...
            if (trailing_sl) {
                if (atr_strategy == USE_ATR && atr_sl_multiplier > 0) {
                    // Adjust for bid at exit
                    new_stop_loss = NormalizeDouble(RoundToTick(tick.bid - (current_atr * atr_sl_multiplier)), decimal);
                } else if (SL > 0) {
                    new_stop_loss = NormalizeDouble(RoundToTick(tick.bid - SL), decimal);
                }
            ...
            if (trailing_tp) {
                if (atr_strategy == USE_ATR && atr_tp_multiplier > 0) {
                    new_take_profit = NormalizeDouble(RoundToTick(tick.ask + (current_atr * atr_tp_multiplier)), decimal);
                } else if (TP > 0) {
                    new_take_profit = NormalizeDouble(RoundToTick(tick.ask + TP), decimal);
                }
            ...
            if (trailing_sl) {
                if (atr_strategy == USE_ATR && atr_sl_multiplier > 0) {
                    new_stop_loss = NormalizeDouble(RoundToTick(tick.ask + (current_atr * atr_sl_multiplier)), decimal);
                } else if (SL > 0) {
                    new_stop_loss = NormalizeDouble(RoundToTick(tick.ask + SL), decimal);
                }
            ...
            if (trailing_tp) {
                if (atr_strategy == USE_ATR && atr_tp_multiplier > 0) {
                    new_take_profit = NormalizeDouble(RoundToTick(tick.bid - (current_atr * atr_tp_multiplier)), decimal);
                } else if (TP > 0) {
                    new_take_profit = NormalizeDouble(RoundToTick(tick.bid - TP), decimal);
                }
...
double fixedPercentageVol() {
    ...
    // Apply balance-based minimum risk floor (only for percentage/optimized modes)
    double minLotsFromFloor = 0.0;
    if (marginPerLot > 0) {
        double minRiskMoney = balance * (min_risk_pct_of_balance / 100.0);
        minLotsFromFloor = minRiskMoney / marginPerLot;
        minLotsFromFloor = stepVol * MathFloor(minLotsFromFloor / stepVol);
        minLotsFromFloor = MathMax(minVol, MathMin(maxVol, minLotsFromFloor));
        finalLots = MathMax(finalLots, minLotsFromFloor);
    }

    PrintFormat(
        "fixedPrecentage Bal=%.2f FreeM=%.2f Risk%%=%.2f%% RiskM=%.2f M/Lot=%.2f Raw=%.4f Final=%.2f (step=%.4f) RiskFloor(balance=%.2f minPct=%.2f%% minLots=%.4f)",
        balance, freeMargin, riskPct, riskMargin, marginPerLot, rawLots, finalLots, stepVol, balance, min_risk_pct_of_balance, minLotsFromFloor);
...
double optimizedVol() {
    ...
    // Apply balance-based minimum risk floor (only for optimized/fixed percentage modes)
    double minLotsFromFloor = 0.0;
    double balance = AccountInfoDouble(ACCOUNT_BALANCE);
    if (marginPerLot > 0) {
        double minRiskMoney = balance * (min_risk_pct_of_balance / 100.0);
    ...
    // 10) Debug print
    PrintFormat(
        "optimizedVol → Price=%.5f  FreeM=%.2f  Risk%%=%d  Margin/Lot=%.2f  RawLots=%.4f  FinalLots=%.2f  Step=%.4f  RiskFloor(balance=%.2f minPct=%.2f%% minLots=%.4f)",
        price, freeMargin, max_risk, marginPerLot, rawLots, finalLots, stepVol, balance, min_risk_pct_of_balance, minLotsFromFloor);
...
double highGrowth() {
    ...
    if (maxDrawdownPercent <= 0)
        maxDrawdownPercent = 0.1; // Avoid zero division
